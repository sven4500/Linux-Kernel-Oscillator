...
// NOTE: https://stackoverflow.com/questions/16758422/is-there-a-non-fatal-equivalent-to-assert-in-c
#define expect(value) do { if (!(value)) { fprintf(stderr, "%s failed in %s:%d\n", #value, __FILE__, __LINE__); } } while(0)
...
int main(int argc, char** argv)
{
    ...
    while (loop)
    {
        ...
        // NOTE: https://stackoverflow.com/questions/2507082/getc-vs-getchar-vs-scanf-for-reading-a-character-from-stdin
        // NOTE: https://stackoverflow.com/questions/58294019/leading-whitespace-when-using-scanf-with-c
        printf("input command (a, r, q): ");
        scanf(" %c", &cmd); // пробел - пропустить все не печатные символы в начале
        //cmd = getchar();
